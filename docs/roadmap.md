...
| L1 | **Controller characterization (Layer 1)** ✅ done — Figure 1 rendered; confirms C_i=0.70·D_i+0.30·B_i, toggle at C_i>0.75, bang-bang (3-doubling ramp, no interior equilibrium). | `controller_sweep.py`, `generate_controller_charts.py` | ✅ | §1d |
| F15 | **Realistic auth-channel (Option 1)** ✅ done — throttle auth (controlQueue) only, data (dataQueue) unthrottled; `-radio-bps` flag (default 250). Unblocks Layer 2. **[Go]** | `udp_broadcast.go`, `rcd.go`, `cmd/rcd/main.go` | auth works below budget, overflows above | F15 |
| 5 | ★ **Static-vs-adaptive comparison (Layer 2)** ✅ done N=1 — adaptive dominates: fixed_fast overflows (QDrops 1506/3148), fixed_slow collapses to 0 auth ≥120Hz, adaptive sustains auth w/ 0 drops. Figures 3–4 rendered. | `comparison_sweep.py`, `generate_comparison_charts.py` | ✅ N=1 | **F18** |
